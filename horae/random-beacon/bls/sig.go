...
import (
	...
	"github.com/ethereum/go-ethereum/horae/random-beacon/blscgo"
	"log"
)
...
func SignatureFromBytes(payload []byte) Signature {
	return Signature{value: payload}
}
...
func VerifySignature(pub []byte, msg []byte, sig []byte) bool {
	...
	pubkey := BytesToPubkey(pub)
	return signature.Sig().Verify(pubkey.PublicKey(), string(msg))
}
